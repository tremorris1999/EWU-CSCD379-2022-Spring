<template>
  <v-container fluid fill-height justify-center>
        ...
        <v-simple-table>
            ...
            <tbody>
                <tr v-for="(player, playerId) in players " :key="playerId">
                    ...
                    <td style="text-align: center">{{ player.averageAttempts.toFixed(2) }}</td>
                    <td style="text-align: center">{{ player.averageSecondsPerGame }}</td>
                </tr>
            </tbody>
        </v-simple-table>
    </v-card-text>
    ...
    <v-card-actions>
        ...
        <v-spacer />
        <v-btn color="primary" @click="getTop10Players"> Get Top 10 Players </v-btn>
    </v-card-actions>
    </v-card>
  </v-container>
</template>
...
export default class leaderboard extends Vue {
    ...
    getTop10Players(){
        this.$axios.get('/api/Players/GetTop10')
        .then(response => {
            this.players = response.data
        })
    }
